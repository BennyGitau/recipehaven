...
const GetRecipe = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setNoResults(false)
    try {
      const response = await axios.post('http://127.0.0.1:5000/get-recipe', { ingredients: ingredients.split(',') });
  ...
  const handlePrint = (recipe) => {
    const printContent = `
      <div>
        <h3>${recipe.title}</h3>
        <img src="${recipe.image}" alt="recipe" style="width:100%; height:auto;" />
        <h4>Ingredients:</h4>
        <ul>
        ...
        <ul>
          ${recipe.neededIngredients.map(ingredient => `<li>${ingredient.originalName} - ${ingredient.amount} ${ingredient.unit}</li>`).join('')}
        </ul>
      </div>
    `;
  ...
  return (
    <Layout>
        <div className="mx-auto p-6 min-h-screen bg-gray-100 w-[90%]">          
          <h2 className="text-3xl font-semibold mb-1 text-orange-400">Didn't find a good recipe?😞 No worry</h2>
          <p className='mb-4 text-xl text-green-400'>Make one from the ingredients you have🥳</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {suggestedRecipes.map((recipe, index) => (
              <div key={index} className="bg-gray-100 text-gray-700 rounded-lg shadow overflow-hidden flex flex-col justify-between">
                <div className="p-4">
                  ...
                  <ul className="list-disc ">
                    {recipe.neededIngredients.map((ingredient, i) => (
