...
// import demo from "../../assets/imgs/sub1.jpg";
...
const Header = () => {
  ...
  const renderDbLogoutButton = () => (
    <NavLink
      ...
      className="relative inline-flex group font-semibold space-x-1 cursor-pointer justify-center items-center borde border-cyan-300 hover:border-green-600 duration-300 ease-in-out px-4 py-1.5 rounded-2xl"
    >
      <span className="font-semibold text-sm">{_user?.username}</span>
        ...
        <nav className="flex items-center space-x-4 [&_a]:text-[13px] xl:[&_a]:text-sm [&_a]:font-semibold">
          ...
          {isLoggedIn && (
            <>
              ...
              {renderNavLink("/profile", "Profile", BiUserCircle)}
            </>
          )}
